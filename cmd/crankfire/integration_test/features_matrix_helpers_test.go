...
func runLoadTest(t *testing.T, cfg *config.Config, serverURL string) (*metrics.Stats, runner.Result) {
	...
	// Update the config with the actual server URL
	cfgCopy := *cfg
	cfgCopy.TargetURL = serverURL

	// Create the actual requester used in production
	// Note: We pass nil for auth provider and feeder since most tests don't use them
	requester, err := NewRequesterFromConfig(&cfgCopy, collector, nil, nil)
	if err != nil {
	...
	// Clean up requester resources if it has a Close method
	if closer, ok := requester.(interface{ Close() error }); ok {
		closer.Close()
	}
